...
{
 public:
  ProxyTunnel(muduo::net::EventLoop* loop,
              const muduo::net::InetAddress& serverAddr,
              const muduo::net::TcpConnectionPtr& serverConn)
    : client_(loop, serverAddr, serverConn->name()),
      serverConn_(serverConn)
  {
    LOG_INFO << "ProxyTunnel " << serverConn->peerAddress().toIpPort()
             << " <-> " << serverAddr.toIpPort();
  }

  ~ProxyTunnel()
  {
    LOG_INFO << "ProxyTunnelunnel";
  }
  ...
  void setup()
  {
    ...
    client_.setConnectionCallback(
        std::bind(&ProxyTunnel::onClientConnection, shared_from_this(), _1));
    client_.setMessageCallback(
        std::bind(&ProxyTunnel::onClientMessage, shared_from_this(), _1, _2, _3));
    serverConn_->setHighWaterMarkCallback(
        std::bind(&ProxyTunnel::onHighWaterMarkWeak,
                  std::weak_ptr<ProxyTunnel>(shared_from_this()), kServer, _1, _2),
        1024*1024);
  }
 ...
 private:
  ...
  void onClientConnection(const muduo::net::TcpConnectionPtr& conn)
  {
    ...
    if (conn->connected())
    {
      ...
      conn->setHighWaterMarkCallback(
          std::bind(&ProxyTunnel::onHighWaterMarkWeak,
                    std::weak_ptr<ProxyTunnel>(shared_from_this()), kClient, _1, _2),
          1024*1024);
    ...
    if (which == kServer)
    {
      if (serverConn_->outputBuffer()->readableBytes() > 0)
      {
        ...
        serverConn_->setWriteCompleteCallback(
            std::bind(&ProxyTunnel::onWriteCompleteWeak,
                      std::weak_ptr<ProxyTunnel>(shared_from_this()), kServer, _1));
      }
    }
    else
    {
      if (clientConn_->outputBuffer()->readableBytes() > 0)
      {
        ...
        clientConn_->setWriteCompleteCallback(
            std::bind(&ProxyTunnel::onWriteCompleteWeak,
                      std::weak_ptr<ProxyTunnel>(shared_from_this()), kClient, _1));
      }
    }
  }

  static void onHighWaterMarkWeak(const std::weak_ptr<ProxyTunnel>& wkTunnel,
                                  ServerClient which,
                                  const muduo::net::TcpConnectionPtr& conn,
                                  size_t bytesToSent)
  {
    std::shared_ptr<ProxyTunnel> tunnel = wkTunnel.lock();
    if (tunnel)
    {
  ...
  static void onWriteCompleteWeak(const std::weak_ptr<ProxyTunnel>& wkTunnel,
                                  ServerClient which,
                                  const muduo::net::TcpConnectionPtr& conn)
  {
    std::shared_ptr<ProxyTunnel> tunnel = wkTunnel.lock();
...
typedef std::shared_ptr<ProxyTunnel> TunnelPtr;
